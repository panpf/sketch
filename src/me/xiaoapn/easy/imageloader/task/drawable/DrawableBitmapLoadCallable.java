...
package me.xiaoapn.easy.imageloader.task.drawable;

import java.util.concurrent.Callable;
import java.util.concurrent.locks.ReentrantLock;

import me.xiaoapn.easy.imageloader.Configuration;
import me.xiaoapn.easy.imageloader.decode.DrawableInputStreamCreator;
...
public class DrawableBitmapLoadCallable implements Callable<BitmapDrawable> {
	private Request request;
	private Configuration configuration;
	private ReentrantLock reentrantLock;
	
	public DrawableBitmapLoadCallable(Request request, ReentrantLock reentrantLock, Configuration configuration) {
	...
	public BitmapDrawable call() throws Exception {
		...
		try{
			...
			if(bitmapDrawable == null){
				//解码
				Bitmap bitmap = configuration.getBitmapDecoder().decode(new DrawableInputStreamCreator(configuration.getContext(), Scheme.DRAWABLE.crop(request.getImageUri())), request.getTargetSize(), configuration, request.getName());
