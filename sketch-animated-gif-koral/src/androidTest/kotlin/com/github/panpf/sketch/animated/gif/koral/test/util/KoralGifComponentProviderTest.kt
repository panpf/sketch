package com.github.panpf.sketch.animated.gif.koral.test.util
...
class KoralGifComponentProviderTest {

    @Test
    fun testAddAndDisabled() {
        val context = getTestContext()
        val componentProvider = KoralGifComponentProvider()
        assertEquals(
            expected = null,
            actual = componentProvider.addFetchers(context)
        )
        assertEquals(
            expected = listOf(KoralGifDecoder.Factory()),
    ...
    fun testEqualsAndHashCode() {
        val element1 = KoralGifComponentProvider()
        val element11 = KoralGifComponentProvider()
    ...
    fun testToString() {
        assertEquals(
            expected = "KoralGifComponentProvider",
            actual = KoralGifComponentProvider().toString()
        )
    }
}