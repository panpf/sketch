...
> [!WARNING]
> * 由于在 jvm 和 macos 平台 Sketch 默认依赖程序的运行路径来构建下载缓存和结果缓存的目录，运行路径一旦发生变化时就会导致下载缓存和结果缓存失效，所以请主动在初始化
    Sketch 时配置一个稳定的下载缓存和结果缓存的目录。
> * 请参考文档 [下载缓存][download_cache] 和 [结果缓存][result_cache] 来了解如何配置下载缓存和结果缓存的目录。
