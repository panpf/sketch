...
/**
 * Cooperate with [ComponentLoader] to achieve automatic registration [BlurHashUriFetcher] and [BlurHashDecoder]
 *
 * @see com.github.panpf.sketch.blurhash.jvm.test.util.BlurHashComponentProviderJvmTest
 */
@Keep
actual class BlurHashComponentProvider : ComponentProvider {

    actual override fun addFetchers(context: PlatformContext): List<Fetcher.Factory>? {
        return listOf(BlurHashUriFetcher.Factory())
    }

    actual override fun addDecoders(context: PlatformContext): List<Decoder.Factory>? {
        return listOf(BlurHashDecoder.Factory())
    }
    ...
    override fun toString(): String = "BlurHashComponentProvider"
}