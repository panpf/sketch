...
class AnimatedDecodeUtilsTest {
    ...
    fun testIsWebP() = runTest {
        ...
        val webpFetchResult = ImageRequest(context, MyImages.webp.uri).fetch(sketch)
        val animWebpFetchResult = ImageRequest(context, MyImages.animWebp.uri).fetch(sketch)
        val jpegFetchResult = ImageRequest(context, MyImages.jpeg.uri).fetch(sketch)
    ...
    fun testIsHeif() = runTest {
        ...
        val animWebpFetchResult = ImageRequest(context, MyImages.animWebp.uri).fetch(sketch)
        val jpegFetchResult = ImageRequest(context, MyImages.jpeg.uri).fetch(sketch)
        val heicFetchResult = ImageRequest(context, MyImages.heic.uri).fetch(sketch)
    ...
    fun testIsAnimatedHeif() = runTest {
        ...
        val animWebpFetchResult = ImageRequest(context, MyImages.animWebp.uri).fetch(sketch)
        val jpegFetchResult = ImageRequest(context, MyImages.jpeg.uri).fetch(sketch)
        val animHeifFetchResult = ImageRequest(context, MyImages.animHeif.uri).fetch(sketch)
        val heicFetchResult = ImageRequest(context, MyImages.heic.uri).fetch(sketch)
