...
fun PhotoList2(
    ...
    SwipeRefresh(
        ...
        if (LayoutMode.valueOf(photoListLayoutMode) == LayoutMode.GRID) {
            PhotoGrid2(lazyPagingItems, animatedPlaceholder, onClick)
        } else {
            PhotoStaggeredGrid2(lazyPagingItems, animatedPlaceholder, onClick)
        }
    }
}

@Composable
private fun PhotoGrid2(
...
@Composable
private fun PhotoStaggeredGrid2(
    lazyPagingItems: LazyPagingItems<Photo>,
    animatedPlaceholder: Boolean,
    onClick: (items: List<Photo>, photo: Photo, index: Int) -> Unit
) {
