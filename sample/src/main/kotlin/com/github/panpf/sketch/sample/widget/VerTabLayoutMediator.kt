...
import com.github.panpf.sketch.sample.widget.VerticalTabLayout.OnTabSelectedListener
import com.github.panpf.sketch.sample.widget.VerticalTabLayout.Tab
import java.lang.ref.WeakReference

/**
 * A mediator to link a VerticalTabLayout with a ViewPager2. The mediator will synchronize the ViewPager2's
 * position with the selected tab when a tab is selected, and the VerticalTabLayout's scroll position when
 * the user drags the ViewPager2. TabLayoutMediator will listen to ViewPager2's OnPageChangeCallback
 * to adjust tab when ViewPager2 moves. TabLayoutMediator listens to VerticalTabLayout's
 ...
 * the mediator object, [.attach] will link the VerticalTabLayout and the ViewPager2 together. When
 ...
 * [.attach] call. Changing the ViewPager2 or VerticalTabLayout will require a new instantiation of
 * TabLayoutMediator.
 */
class VerTabLayoutMediator(
    private val tabLayout: VerticalTabLayout,
    ...
    /**
     * Returns whether the [VerticalTabLayout] and the [ViewPager2] are linked together.
     */
    ...
    /**
     * Link the VerticalTabLayout and the ViewPager2 together. Must be called after ViewPager2 has an adapter
...
//    tabLayout.setScrollPosition(viewPager.getCurrentItem(), 0f, true);
        tabLayout.setScrollPosition(viewPager.currentItem, 0f)
    }

    /**
     * Unlink the VerticalTabLayout and the ViewPager. To be called on a stale TabLayoutMediator if a new one
    ...
    fun interface TabConfigurationStrategy {
        /**
         * Called to configure the tab for the page at the specified position. Typically calls [ ][VerticalTabLayout.Tab.setText], but any form of styling can be applied.
         *
     ...
     * provided [VerticalTabLayout] so that the tab position is kept in sync.
     *
     *
     * This class stores the provided VerticalTabLayout weakly, meaning that you can use [ ][ViewPager2.registerOnPageChangeCallback] without removing the
     * callback and not cause a leak.
     */
    private class TabLayoutOnPageChangeCallback internal constructor(tabLayout: VerticalTabLayout) :
        OnPageChangeCallback() {
        private val tabLayoutRef: WeakReference<VerticalTabLayout>
    ...
    /**
     * A [VerticalTabLayout.OnTabSelectedListener] class which contains the necessary calls back to the
