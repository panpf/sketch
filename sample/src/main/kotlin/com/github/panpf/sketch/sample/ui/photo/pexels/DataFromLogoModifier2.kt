...
fun Modifier.dataFromLogo2(state: AsyncImageState): Modifier {
    return this.then(DataFromLogoElement2(state))
}

internal data class DataFromLogoElement2(
    val state: AsyncImageState
) : ModifierNodeElement<DataFromLogoNode2>() {

    override fun create(): DataFromLogoNode2 {
        return DataFromLogoNode2(state)
    }

    override fun update(node: DataFromLogoNode2) {
        node.update(state)
    }

    override fun InspectorInfo.inspectableProperties() {
        name = "DataFromLogo"
        properties["dataFrom"] = state.loadState
            ?.let { (it as? LoadState.Success) }
            ?.result?.dataFrom?.name
            ?: "null"
        properties["loadState"] = state.loadState?.name ?: "null"
    }
}

internal class DataFromLogoNode2(
    ...
    override fun ContentDrawScope.draw() {
        ...
        if (loadState != null && loadState is LoadState.Success) {
            val path = getPath(size)
            val dataFrom = loadState.result.dataFrom
