 ...
 *
 * @param appVersion       Version, used to delete the old cache, update this value when you want to actively delete the old cache
...
class LruDiskCache constructor(
    ...
    val internalVersion: Int = 1,   // Controlled by sketch, sketch fixes the wrong disk cache generated by the old version and will modify this version to update the cache
) : DiskCache {
    ...
    init {
        require(appVersion in 1.rangeTo(Short.MAX_VALUE)) {
            "The value range for 'version' is 1 to ${Short.MAX_VALUE}"
        }
        require(internalVersion in 1.rangeTo(Short.MAX_VALUE)) {
            "The value range for 'internalVersion' is 1 to ${Short.MAX_VALUE}"
        }
    }
    ...
    override fun toString(): String =
        "$MODULE(maxSize=${maxSize.formatFileSize()},version=${appVersion},directory='${directory.path}')"
