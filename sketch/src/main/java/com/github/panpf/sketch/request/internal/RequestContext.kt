...
class RequestContext constructor(firstRequest: ImageRequest, val size: Size) {
    ...
    val key: String
        get() = _key ?: request.newKey(size).apply {
            _key = this
        }
    ...
    val cacheKey: String
        get() = _cacheKey ?: request.newCacheKey(size).apply {
            _cacheKey = this
        }
