 ...
 * 显示请求
 */
public class DefaultDisplayRequest extends DefaultLoadRequest implements DisplayRequest {
        ...
        if (displayOptions.isCacheInMemory()) {
            ...
            if (cacheDrawable != null) {
                ...
                if (!recycleDrawable.isRecycled()) {
                    if (Sketch.isDebugMode()) {
                        Log.i(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runLoad", " - ", "from memory get drawable", " - ", recycleDrawable.getInfo(), " - ", getAttrs().getName()));
                    }
                    ...
                    if (Sketch.isDebugMode()) {
                        Log.e(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runLoad", "bitmap recycled", " - ", recycleDrawable.getInfo(), " - ", getAttrs().getName()));
                    }
                }
            }
        }
    ...
    protected void requestLevelIsLocal() {
        if (displayOptions.getRequestLevelFrom() == RequestLevelFrom.PAUSE_DOWNLOAD) {
            if (Sketch.isDebugMode()) {
                Log.w(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runDispatch", " - ", "canceled", " - ", "pause download", " - ", getAttrs().getName()));
            }
            postRunPauseDownload();
        } else {
            if (Sketch.isDebugMode()) {
                Log.w(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runDispatch", " - ", "canceled", " - ", "requestLevel is local", " - ", getAttrs().getName()));
            }
    ...
    protected void loadCompleted() {
        ...
        if (loadResult.getBitmap() != null) {
            if (loadResult.getBitmap().isRecycled()) {
                if (Sketch.isDebugMode()) {
                    Log.e(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runLoad", " - ", "bitmap recycled", " - ", loadResult.getGifDrawable().getInfo(), " - ", getAttrs().getName()));
                }
                failed(FailedCause.DECODE_FAIL);
                return;
            }
        ...
        if (loadResult.getGifDrawable() != null) {
            if (loadResult.getGifDrawable().isRecycled()) {
                if (Sketch.isDebugMode()) {
                    Log.e(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runLoad", " - ", "gif drawable recycled", " - ", loadResult.getGifDrawable().getInfo(), " - ", getAttrs().getName()));
                }
                failed(FailedCause.DECODE_FAIL);
                return;
            }
            ...
            if (Sketch.isDebugMode()) {
                Log.d(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runLoad", " - ", "new gif drawable", " - ", loadResult.getGifDrawable().getInfo(), " - ", getAttrs().getName()));
            }
    ...
    protected void runCompletedInMainThread() {
        ...
        if (completedDrawable != null) {
            ...
            if (Sketch.isDebugMode()) {
                Log.d(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runCompletedInMainThread", " - ", "completedDrawable is null", " - ", getAttrs().getName()));
            }
        }
    ...
    protected void runFailedInMainThread() {
        ...
        if (failedDrawable != null) {
            ...
            if (Sketch.isDebugMode()) {
                Log.w(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runFailedInMainThread", " - ", "failedDrawable is null", " - ", getAttrs().getName()));
            }
        }
    ...
    private void runPauseDownloadOnMainThread() {
        if (isCanceled()) {
            if (Sketch.isDebugMode()) {
                Log.w(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runPauseDownloadOnMainThread", " - ", "canceled", " - ", getAttrs().getName()));
            }
            return;
        }
        ...
        if (pauseDownloadDrawable != null) {
            ...
            if (Sketch.isDebugMode()) {
                Log.d(Sketch.TAG, SketchUtils.concat(NAME, " - ", "runPauseDownloadOnMainThread", " - ", "pauseDownloadDrawable is null", " - ", getAttrs().getName()));
            }
        }
