...
public abstract class BasePool<V> implements Pool<V> {
    ...
    public V get(int size) {
        ...
        synchronized (this) {
            ...
            if (bucket != null) {
                ...
                if (value != null) {
                    ...
                    logStats();
                    if (Log.isLoggable(TAG.getSimpleName(), Log.VERBOSE)) {
                        Log.v(TAG.getSimpleName(), String.format("get (reuse) (object, size) = (%x, %s)",
        ...
        synchronized (this) {
            ...
            if (Log.isLoggable(TAG.getSimpleName(), Log.VERBOSE)) {
                Log.v(
                        TAG.getSimpleName(), String.format(
     ...
     */
    void trimToNothing() {
        final List<Bucket<V>> bucketsToTrim = new ArrayList<Bucket<V>>(mBuckets.size());
    ...
    synchronized Bucket<V> getBucket(int bucketedSize) {
        ...
        // create a new bucket
        if (Log.isLoggable(TAG.getSimpleName(), Log.VERBOSE)) {
            Log.v(TAG.getSimpleName(), String.format("creating new bucket %s", bucketedSize));
        }
