...
./gradlew \
  ...
  samples:jvmApp:packageReleaseDistributionForCurrentOS \
...
xcodebuild \
  ...
  -scheme iosApp \
  -destination 'generic/platform=iOS Simulator' ARCHS=arm64 CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO \
  -derivedDataPath samples/shared/build/ios/outputs/
...
echo "JVM distribution is written to $(pwd)/samples/jvmApp/build/compose/binaries/main/"
