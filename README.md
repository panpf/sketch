...
下面是三种方法所支持的属性（'-'代表不支持，非'-'代表支持并且默认值是什么）
...
各属性的作用请参考[配置各种属性](docs/wiki/options.md)
...
>* [显示（display()）、加载（load()）和下载（download()））](docs/wiki/load_download.md)
>* [配置各种属性](docs/wiki/options.md)
