...
>* [使用``maxsize``功能防止加载过大的图片以``节省内存``](https://github.com/xiaopansky/Spear/wiki/ImageSize)
>* [使用``resize``功能使加载到内存的图片``同ImageView的尺寸一样``，这样可最大限度的``节省内存``]()
>* [自定义``InSampleSize``计算规则或``自定义图片解码器``（ImageDecoder）](https://github.com/xiaopansky/Spear/wiki/ImageDecoder)
>* [使用ImageProcessor处理图片，例如处理成``圆形``的、``椭圆形``的或者加上``倒影效果``](https://github.com/xiaopansky/Spear/wiki/ImageProcessor)
>* [使用ImageDisplayer以有趣的动画效果显示图片，例如以``渐变``或``缩放``的方式显示](https://github.com/xiaopansky/Spear/wiki/ImageDisplayer)
