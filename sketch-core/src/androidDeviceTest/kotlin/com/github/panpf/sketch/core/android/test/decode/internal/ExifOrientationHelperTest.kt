...
class ExifOrientationHelperTest {
    ...
    fun testReadExifOrientation() = runTest {
        ...
        assertEquals(
            ...
            ComposeResImageFiles.jpeg.toDataSource(context).readExifOrientation()
        )

        assertEquals(
            ExifOrientationHelper.UNDEFINED,
            ComposeResImageFiles.webp.toDataSource(context).readExifOrientation()
        )

        ComposeResImageFiles.clockExifs.forEach {
            assertEquals(
    ...
    @Test
    fun testReadExifOrientationWithMimeType() = runTest {
        val context = getTestContext()
        ...
        assertEquals(
            ExifOrientationHelper.UNDEFINED,
            ComposeResImageFiles.jpeg.toDataSource(context)
                .readExifOrientationWithMimeType("image/bmp")
        )

        assertEquals(
            ExifOrientationHelper.UNDEFINED,
            ComposeResImageFiles.webp.toDataSource(context)
    ...
    @Test
    fun testApplyToBitmap() = runTest {
        val context = getTestContext()
        val inBitmap = ComposeResImageFiles.jpeg
    ...
    @Test
    fun testAddAndApplyToBitmap() = runTest {
        val context = getTestContext()
        val inBitmap = ComposeResImageFiles.jpeg
            .toDataSource(context).openSource()
            .buffer().inputStream().use {
