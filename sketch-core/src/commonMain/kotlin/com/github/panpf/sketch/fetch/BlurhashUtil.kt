...
//        return composeBitmap(width, height, numCompX, numCompY, colors)
    ...
    fun decodeInt(blurHash: String?, width: Int, height: Int, punch: Float = 1f): IntArray? {
        if (blurHash == null || blurHash.length < 6) {
            return null
        }
        val numCompEnc = decode83(blurHash, 0, 1)
    ...
    private fun composeBitmapAsByteArray(
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                ...
                // Write to byte array (RGBA format)
                val base = pixelIndex * 4
                byteArray[base] = red.toByte()
                byteArray[base + 1] = green.toByte()
                byteArray[base + 2] = blue.toByte()
                byteArray[base + 3] = alpha.toByte()
