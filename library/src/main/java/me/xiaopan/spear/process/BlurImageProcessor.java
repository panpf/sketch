 ...
 * 高斯模糊图片处理器
 */
public class BlurImageProcessor extends CutImageProcessor{
    private int radius;
    private boolean isDarkHandle;

    /**
     * @param radius 模糊半径，取值为0到100，通常建议为15
     * @param isDarkHandle 是否让模糊后的图片看起来更暗一些，实现原理就是加上一层#88000000颜色。常用于页面背景，因为太亮的背景会影响页面上展示的内容
     */
    public BlurImageProcessor(int radius, boolean isDarkHandle) {
        this.radius = radius;
        this.isDarkHandle = isDarkHandle;
    }

    public BlurImageProcessor(int radius) {
        this(radius, false);
    }
    ...
    public Bitmap process(Bitmap bitmap, ImageSize resize, ImageView.ScaleType scaleType) {
        ...
        Bitmap blurBitmap = createBlurBitmap(resizeBitmap, radius);
    ...
    public String getFlag() {
        return "BlurImageProcessor(radius="+radius+";isDarkHandle="+isDarkHandle+")";
    }
