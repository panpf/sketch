...
class HeifAnimatedDrawableDecoderTest {
            ...
            build().apply {
                Assert.assertEquals(
                    "ComponentRegistry(" +
                            "fetcherFactoryList=[]," +
                            "bitmapDecoderFactoryList=[]," +
                            "drawableDecoderFactoryList=[HeifAnimatedDrawableDecoder]," +
                            "requestInterceptorList=[]," +
                            "bitmapDecodeInterceptorList=[]," +
                            "drawableDecodeInterceptorList=[]" +
                            ")",
            ...
            build().apply {
                Assert.assertEquals(
                    "ComponentRegistry(" +
                            "fetcherFactoryList=[]," +
                            "bitmapDecoderFactoryList=[]," +
                            "drawableDecoderFactoryList=[HeifAnimatedDrawableDecoder,HeifAnimatedDrawableDecoder]," +
                            "requestInterceptorList=[]," +
                            "bitmapDecodeInterceptorList=[]," +
                            "drawableDecodeInterceptorList=[]" +
                            ")",
    ...
    fun testFactory() {
        ...
        // normal
        DisplayRequest(context, AssetImages.animHeif.uri).let {
            val fetchResult =
                FetchResult(
        ...
        DisplayRequest(context, AssetImages.animHeif.uri) {
    ...
    fun testDecodeDrawable() {
        ...
        val request = DisplayRequest(context, AssetImages.animHeif.uri) {
        ...
        factory.create(sketch, request.toRequestContext(), fetchResult)!!
            .let { runBlocking { it.decode() }.getOrThrow() }.apply {
                ...
                val animatedImageDrawable =
                    ((this.drawable as SketchAnimatableDrawable).drawable as com.github.panpf.sketch.drawable.ScaledAnimatedImageDrawable).drawable
                Assert.assertEquals(-1, animatedImageDrawable.repeatCount)
            }

        val request1 = DisplayRequest(context, AssetImages.animHeif.uri) {
            repeatCount(3)
            resizeSize(100, 100)
        }
